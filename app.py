...
def reading_network():
    ...
    while True:
        try:
            #print(socket_sub.recv())
            [topicValue,data] = socket_sub.recv_multipart()
            topic = topicValue.decode()
            if topic == "chain":
                jdata = json.loads(data.decode())
                print(jdata)
                if(blockchain and blockchain.nbBlock <= jdata["length"]):
                    blockchain = Blockchain(difficulty, jdata["chain"])
                    ConnectionWrite.write_block.emit(str(blockchain.chain[-1]))
            if topic == "PeerAddress":
                print(data)
            if topic == "chainfalse":
                print(data)
...
class Tx_Dialog(QtWidgets.QDialog):
    ...
    def working_click(self):
        ...
        socket.send_multipart([b'Transactions', json.dumps(self.get_values()).encode()])
        self.accept()
...
class Peer_Dialog(QtWidgets.QDialog):
    ...
    def working_click(self):
        # add send tx to other peers function here
        #socket.send_multipart([b'PeerAddress', self.peer_address])
        self.accept()
...
class MyWidget(QtWidgets.QWidget):
    ...
    def mine_call(self):
        ...
        if blockchain.verifyChain() == True:
            print("Blockchain verifie\n")
            self.define_block(blockchain.chain[-1])
            b = Block("Admin", "info", blockchain.nbBlock, blockchain.chain[-1].hash, address, [], time.time())
            blockchain.addBlock(b)
            chain = blockchain.to_dict()
            chain2 = json.dumps({"length": blockchain.nbBlock, "chain": chain}).encode()
            socket.send_multipart([b'chain', chain2])
            """ print(chain2)
            print(json.dumps(chain2)) """
        else :
            socket.send_multipart([b'chainfalse', json.dumps({"result":"Blockchain Not Verify"}).encode()])
    ...
    def define_tx(self, elem):
        ...
        blockchain.add_transaction(elem["address"], address, elem["amount"], wallet)
